...
<section class="w-full flex justify-center my-8">
  <div class="card clock my-4 rounded-xl">
    ...
    <div role="tablist" class="tabs tabs-boxed m-4">
      <button role="tab" class="tab" on:click={() => setup(POMODORO_MINUTES, ModeState.FOCUS)}>Focus time</button>
      <button role="tab" class="tab tab-active" on:click={() => setup(SHORT_BREAK_MINUTES, ModeState.REST)}>Short break</button>
      <button role="tab" class="tab" on:click={() => setup(LONG_BREAK_MINUTES, ModeState.REST)}>Long break</button>
    </div>
